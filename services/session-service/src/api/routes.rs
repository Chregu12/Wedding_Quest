...
pub fn routes(state: AppState) -> axum::Router {
    RouterBuilder::new()
        ...
        .route("/sessions", get(session_handlers::list_sessions).post(session_handlers::create_session))
        .route("/sessions/{code}", get(session_handlers::get_session).put(session_handlers::update_session))
        .route("/sessions/{code}/start", post(session_handlers::start_session))
        .route("/sessions/{code}/players", get(player_handlers::get_lobby).delete(player_handlers::clear_players))
        ...
        .route(
            ...
            delete(question_handlers::delete_question).put(question_handlers::update_question),
        )
        .route(
